...
func TestBuildRequestTitle(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			issueNumber:  225,
			crossRepoRef: "example.com/engineering/platform/issues#225",
			issueTitle:   "some random title",
			want:         "example.com/engineering/platform/issues#225: some random title",
		},
		{
...
func TestCreateMergeRequest_CrossRepo(t *testing.T) {
	...
	_, err := CreateMergeRequest(CreateMergeRequestParams{
		...
		TargetBranch:      "main",
		IssueNumber:       225,
		CrossRepoIssueRef: "example.com/engineering/platform/issues#225",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	wantTitle := "example.com/engineering/platform/issues#225: some random title"
	if got.Title != wantTitle {
		t.Errorf("Title = %q, want %q", got.Title, wantTitle)
	}
	wantDesc := "Closes example.com/engineering/platform/issues#225"
